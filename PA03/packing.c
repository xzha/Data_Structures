...
int tree_save(const char * filename, int blocks, Node * tree)
{
    ...
    for (i = 1; i <= blocks; i++)
    {
        fprintf(fh,"%d %le %le %le %le\n", i, tree[i].width, tree[i].height, tree[i].xcoord, tree[i].ycoord);
    }
...
void inorder_print(Node * tree, Node * root)
{
    ...
    printf("%d ", root->thisnode);
    inorder_print(tree, &tree[root->rcnode]);
    return;
}
...
void preorder_print(Node * tree, Node * root)
{
    ...
    preorder_print(tree, &tree[root->rcnode]);
    return;
}
...
void tree_pack_coordinate(Node * tree, Node * root)
{
    ...
    tree_pack_coordinate(tree, &tree[root->rcnode]);
    return;
}
...
Node * tree_load(const char * filename, int * blocks, int * nodes)
{
    ...
    {
        g = fscanf(fh,"%d %d %d %d %c %lf %lf", &tree[i].thisnode, &tree[i].parnode, &tree[i].lcnode, &tree[i].rcnode, &tree[i].cutline,
               &tree[i].width, &tree[i].height);
        i++;
    }
    ...
    {
        g = fscanf(fh,"%d %d %d %d %c %*c %*c", &tree[i].thisnode, &tree[i].parnode, &tree[i].lcnode, &tree[i].rcnode, &tree[i].cutline);
        tree[i].width = 0;
        tree[i].height = 0;
        i++;
    }
